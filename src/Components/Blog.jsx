...
const Blog = ({ title, description, bannerImage, publishedDateTime, publisherName, email, photo, category }) => {
    // State for managing the love and bookmark status
    const [isLoved, setIsLoved] = useState(false);
    const [isBookmarked, setIsBookmarked] = useState(false);
    const [loveCount, setLoveCount] = useState(0);

    // Toggle the love button background color
            ...
            <div className="p-4">
                ...
                <div className='flex justify-between items-center mt-6'>
                    <div className="flex items-center gap-5">
                        ...
                        >
                            {isLoved ? <AiFillHeart className="text-pink-500" size={24} /> : <AiOutlineHeart className="text-gray-600" size={24} />}
                        ...
                        >
                            {isBookmarked ? <BsBookmarkFill className="text-orange-500" size={24} /> : <BsBookmark className="text-gray-600" size={24} />}
                        </button>
                    </div>
